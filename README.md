...
把**任意一本扫描版外语教材**变成可点读的电子书：完整保留原书版面，
鼠标点到任意单词就能**听到发音**并**看到母语释义**。

    PDF 由使用者自己提供，本仓库只提供工具。

**实测案例**：用《新版现代西班牙语》第一册（191 MB，305 页纯扫描件，
无文字层）跑完全流程，产出 35572 个可点词、83.8% 的离线查得率。
下面所有数字都来自这个案例，但流水线本身不绑定任何一本书。

## 它能做什么

| 环节 | 怎么实现 | 是否与具体教材绑定 |
|---|---|---|
| 解析 PDF | 自写纯标准库解析器，从 DCTDecode 流里抽出页面图 | 通用 |
| 文字识别 | Windows 内置 OCR（`Windows.Media.Ocr`），双语言引擎，**词级坐标** | 通用 |
| OCR 纠错 | 针对 Windows OCR 的**系统性错误模式**的规则引擎 + 分级词库 | 通用 |
| 词表页识别 | 按**版式特征与标题词**自动定位词汇表，不依赖页码 | 通用 |
| 词典构建 | 课本词表 + 通用常用词库 + 在线查词兜底 | 通用 |
| 词形还原 | 西语动词变位引擎（规则变位 + 142 个不规则动词） | 语种相关 |
| 阅读器 | 单文件零依赖前端：点读 / 整句朗读 / 搜索 / 生词本 | 通用 |

**第三方库一个都不需要** —— 整条流水线只用 Python 标准库。
...
**扫描版教材没有文字层。** 案例用的这本 191 MB PDF 里是 305 张 JPEG 扫描图，
没有任何 `/Font` 或 `/ToUnicode` 表 —— 也就是说，**任何 PDF 阅读器都选不中、
搜不到里面的文字**，更别说逐词点读。

这是绝大多数老旧教材、外文原版书、影印资料的共同状况，所以整个工具的第一步
就是把它"读"出来。完整流程：
...
1. **发音依赖 Edge + 联网**。Windows 默认没有西语语音包，离线环境下西语发音不可用
   （中文语音读不了西语）。装一个西班牙语语音包可以彻底离线。
2. **OCR 残留约 1%**。约 300 个词仍有识别错误，集中在语法表、插图说明等区域
   （如 `[A`、`E]`、`Valk2nte`）。点读时会显示「本地词库未收录」，可用在线查词兜底。
3. **释义有少量缺失**。词表页的释义列 OCR 偶有丢字（如 `siempre` 的释义读成了 `疋`），
   已用部首噪声过滤清理，但无法完全复原。
4. **页码即扫描页序**，与书本印刷页码可能相差 1–2 页（书前有扉页、目录）。
5. 首次打开需加载 4.4 MB 的 `pages.json` 与 2.5 MB 的 `lemmas.json`，本机约 1–2 秒。
6. **词表页识别依赖版式**。手写体、竖排、或整页只有三五条词目的书，三条线索都可能落空；
   此时词典退回常用词库 + 在线查词（不报错，但释义质量下降）。
...
└─ tools/
   ...
   ├─ check_vocab_detect.py  词表页识别回归测试（精确率/召回率）
   ├─ calibrate_vocab_detect.py  识别阈值的校准工具（换新书时用）
   ├─ verify.py              残留错误模式统计
...
`tools/check_tts_server.py` 另外对 `/tts` 端点做 25 项检查，分两类：

- **确定性**：输入校验（缺参/超长/路径穿越/HTML 注入全部 400）、
  HEAD 不触发下载、限制长度、静态资源不受影响
- **实拉**：真的向 Google 拉一个词，确认拿到的是有效 MP3（帧同步位正确）、
  确认它落盘、确认第二次请求由磁盘命中且**字节与第一次完全一致**、
  确认不同词互不干扰

```
OK   返回的是真实 MP3 音频（前两字节 fff3）
OK   音频已落盘：cache/tts/4d852bdd6de7dd4e1ca2.mp3
OK   磁盘文件与响应字节完全一致
OK   第二次走缓存：X-TTS-Cache=hit
OK   两次返回字节完全一致（缓存没有损坏）
```

上游不可达时实拉部分会**跳过而不是失败**，所以离线环境下这个检查仍然有用。
...
**不需要改任何代码。** 把新书命名为 `textbook.pdf` 放到根目录，重跑即可：

```powershell
python tools\run_all.py
```

流水线里唯一曾经写死过页码的地方（`GLOSARIO_PAGES` / `VOCAB_PAGES`）已经换成
**自动识别**，见下一节。

### 词表页是怎么自动找到的

书籍的词汇表页是词典质量的关键，但不同书的位置、版式、标题语言都不同。
`tools/vocab_pages.py` 用三条线索定位，**完全不看页码**：

| 线索 | 判据 | 在本案例的表现 |
|---|---|---|
| ① 标题词 | 页面出现 `VOCABULARIO` / `GLOSARIO` / `词汇表` / `Wortschatz` / `単語` … | 命中 22 页，**零误报** |
| ② 邻居传播 | 已识别页的左右 ±2 页，若词性缩写密度接近、短行占比高、词数够多 | 补齐 5 页 |
| ③ 版式兜底 | 一个标题词都认不出时：密度 ≥ 0.18 且整页 ≥ 200 词且几乎全是短行 | 10 页全中，零误报 |

最终 **27/27 命中，0 误报，0 漏检**（精确率与召回率都是 100%），
而且把 OCR 记录顺序整个反转后结果完全一致 —— 证明它真的与页码无关。

阈值不是拍脑袋定的，`tools/calibrate_vocab_detect.py` 会打印不同阈值下的
命中/误报/漏检，换新书想重新校准就跑它。回归测试见 `tools/check_vocab_detect.py`。

**认不出词表页也不会失败。** 词典会自动退回「通用常用词库 + 在线查词」，
阅读器照常可用，只是没有课本官方释义 —— 这是优雅降级，不是报错。

### 限制

- **必须在 Windows 上跑**。OCR 用的是系统自带的 `Windows.Media.Ocr`，
  Mac / Linux 没有。想在别的系统上跑得换 OCR 引擎，并重写 `tools/repair.py`
  的纠错规则 —— 那套规则是针对 **Windows OCR 的错误模式**总结的，换引擎就不适用。
- **词形还原目前只有西班牙语**。换成法语/德语，`conjugate.py` 与
  `dict_seed.json` 需要替换成对应语言的版本；其余环节都是通用的。
- **释义语言假定为中文**。`extract_dict.py` 靠 Unicode 的汉字区间
  （`\u3400-\u9fff`）从词表页里挑出释义列；换成英释、日释的书需要改这个判据。
